...
import { stripQuiz } from '@/lib/positioning';
...
export async function GET(req) {
  ...
  const rows = await q(
    `SELECT s.id, s.title, s.start_date, s.end_date, s.trainer, s.duration, s.modality, s.quiz
  ...
  const pos = await q('SELECT session_id FROM positionings WHERE trainee_id = $1', [auth.traineeId]);
  const posDone = new Set(pos.map((p) => p.session_id));
  ...
  const sessions = rows.map((s) => {
    ...
    return {
      ...s, quiz: undefined, slots, signedCount: slots.filter((x) => x.signed).length, total: slots.length,
      surveys: { chaud: !!satMap[`${s.id}|chaud`], froid: !!satMap[`${s.id}|froid`] },
      positioning: { done: posDone.has(s.id), quiz: stripQuiz(s.quiz) },
    };
  });
